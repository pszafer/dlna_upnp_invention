...
class Resource(object):
    ...
    def get_additional_info(self,upnp_client=''):
        _,_,content_format,additional_info = self.protocolInfo.split(':')
    ...
    def toElement(self,**kwargs):
        ...
        if kwargs.get('upnp_client','') == 'XBox':
            ...
            if(kwargs.get('parent_container',None) != None and
                url.text.startswith('object.container')):
        ...
        if self.albumArtURI is not None:
            ...
            e1 = ET.SubElement(root, qname('icon',UPNP_NS))
            e1.text = self.albumArtURI
            
        if self.caption is not None:
            g = ET.SubElement(root, qname('dcmInfo', SEC))
            g.text = "CREATIONDATE=0,FOLDER=test,BM=180"
            f = ET.SubElement(root, qname('CaptionInfoEx', SEC))
            f.text = self.caption
            f.attrib["sec:type"] = "srt"
